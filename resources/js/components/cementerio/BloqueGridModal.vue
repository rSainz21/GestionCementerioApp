      ...
      <template v-else>
        ...
        >
          <button
            ...
            @click="openSep(cell.sepultura?.id)"
          >
...
const props = defineProps({
  ...
  bloqueId: { type: Number, default: null },
  bloque: { type: Object, default: null }, // opcional para header/columnas/filas
});
...
const header = computed(() => {
  ...
  const dims = b.filas && b.columnas ? ` · ${b.filas}×${b.columnas}` : '';
  const cod  = b.codigo ? ` (${b.codigo})` : '';
  return `${b.nombre ?? 'Bloque'}${dims}${cod}`;
});
...
async function onShow() {
  const id = Number(props.bloqueId ?? props.bloque?.id);
  ...
  try {
    ...
    error.value = e?.response?.data?.message ?? 'No se pudieron cargar las sepulturas.';
  } finally {
    loading.value = false;
  }
}
