<template>
  <div class="wrap">
    ...
    <DataTable :value="filtered" filterDisplay="row" v-model:filters="colFilters" stripedRows :loading="loading" paginator :rows="20">
      ...
      <Column field="nombre_original" header="Nombre / Razón social" :showFilterMenu="false">
        <template #filter="{ filterModel, filterCallback }">
          <InputText v-model="filterModel.value" @input="filterCallback()" placeholder="Filtrar nombre…" class="col-filter" />
        </template>
        <template #body="{ data }">
          <button type="button" class="nombre-link" @click="abrirDetalleTercero(data)">
      ...
      <Column style="width:52px">
        ...
        <template #body="{ data }">
          <Button icon="pi pi-eye" size="small" text severity="success"
                  v-tooltip.left="'Ver detalle'" @click="abrirDetalleTercero(data)" />
        </template>
      </Column>
    </DataTable>

    <TerceroDetalleModal v-model="terceroDetalleVisible" :tercero="terceroSeleccionado" />
...
import TerceroDetalleModal from './TerceroDetalleModal.vue';
...
// Modal detalle tercero
const terceroDetalleVisible = ref(false);
const terceroSeleccionado = ref(null);

function abrirDetalleTercero(tercero) {
  terceroSeleccionado.value = tercero;
  terceroDetalleVisible.value = true;
}

const colFilters = ref({
  nombre_original: { value: null, matchMode: FilterMatchMode.CONTAINS },
});
...
.concesion-card {
  border: 1px solid #e5e7e6; border-radius: 10px; padding: 12px 16px;
  cursor: pointer; transition: box-shadow .15s, border-color .15s; display: grid; gap: 6px;
}
...
.badge-codigo { background: var(--c2-primary, #118652); color: white; border-radius: 6px; padding: 2px 10px; font-size: 12px; font-weight: 700; }
.difuntos-mini { font-size: 12px; color: #666; }
.ver-detalle { font-size: 12px; color: var(--c2-primary, #118652); font-weight: 600; display: flex; align-items: center; gap: 4px; justify-content: flex-end; }
.nombre-link {
  background: none; border: none; padding: 0; cursor: pointer;
  font-size: 13px; font-weight: 600; color: rgba(23,35,31,.88); text-align: left;
}
