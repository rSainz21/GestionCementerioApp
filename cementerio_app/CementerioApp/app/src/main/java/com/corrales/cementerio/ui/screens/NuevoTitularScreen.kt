                    ...
                    onClick = {
                        ...
                        scope.launch {
                            ...
                            titularResult.onFailure {
                                errorGuardado = "Error al guardar titular: ${it.message}"
                                isLoading = false; return@launch
                            }
                            val titularId = titularResult.getOrNull()?.id
                            // PASO 2: Crear concesión si hay fechas y hay nicho
                            if (titularId != null && codigoNicho.isNotBlank() && fechaInicio.isNotBlank()) {
                                // Buscar unidad por código
                                CementerioRepository.listarTodasUnidades().onSuccess { lista ->
                                    val unidad = lista.find { it.codigo.equals(codigoNicho, ignoreCase = true) }
                                    if (unidad?.id != null) {
                                        val inicio = try {
                                            if (fechaInicio.contains("/"))
                                                LocalDate.parse(fechaInicio, fmtD).format(fmt)
                                            else fechaInicio
                                        } catch (e: Exception) { LocalDate.now().format(fmt) }
                                        val vencimiento = try {
                                            LocalDate.parse(inicio, fmt).plusYears(duracionSelec.toLong()).format(fmt)
                                        } catch (e: Exception) { LocalDate.now().plusYears(10).format(fmt) }
                                        CementerioRepository.crearConcesion(
                                            ConcesionRequest(
                                                unidadEnterramiento = IdWrapper(unidad.id),
                                        ...
                                        CementerioRepository.actualizarEstadoUnidad(unidad.id, "Reservado")
                                    }
                                }
                            }
                            isLoading = false
